...
STORE=ROOT/'audit_store/write_investigation_v2'
RAM=Path('/dev/shm/ist_review_write_investigation_v2')
...
def capture_failures():
    ...
    def preserve_then_unlink(p,*args,**kwargs):
        roots=(HERE,STORE,RAM)
        inside=any(root==p.parent or root in p.parents for root in roots)
        if inside and p.exists() and (p.name.startswith('.writing-') or p.suffix=='.json'):
            import uuid
            evidence=p.with_name('preserved-'+p.name+'-'+uuid.uuid4().hex+'.bin')
            os.link(p,evidence)
            CAPTURES.append({'path':str(evidence),'size_at_capture':p.stat().st_size,'inode':p.stat().st_ino})
        return original_unlink(p,*args,**kwargs)
    Path.unlink=preserve_then_unlink
    try:
        yield
    finally:
...
def child(task):
    ...
    try:
        ...
        report['preserved']=CAPTURES[capture_start:]
...
if __name__=='__main__':
    with capture_failures():
        try:main()
            ...
            __import__('importlib').reload(io)
            io.atomic_json(HERE/('setup_capture.json' if '--child' not in sys.argv else 'child_setup_capture.json'),{'captured':CAPTURES,'traceback':traceback.format_exc()})
            raise
