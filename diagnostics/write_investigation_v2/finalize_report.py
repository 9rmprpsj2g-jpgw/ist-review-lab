"""Summarize completed I/O observations without running further trials."""
import json
import re
import sys
from pathlib import Path
sys.path.insert(0, str(Path(__file__).resolve().parents[2]))
from src.durable_io import atomic_json, write_text, sha256_file
...
def main():
    ...
    for path in sorted(HERE.glob('*.log')):
        ...
        logs.append({'file': path.name, 'sha256': sha256_file(path),
                     'lines_read': len(content.splitlines()),
                     'diagnostic_lines': [line for line in content.splitlines()
                         if re.search(r'Warning|Error|Exception|Operation not permitted|FAIL', line)]})
    atomic_json(HERE/'log_inspection.json', {
        'scope': 'Every current investigation log read completely, not just tails.',
        'logs': logs,
        'attribution': 'Thirteen trial integrity failures, one separate setup truncation, denied ptrace before any traced child. No captured convergence warnings or model fits. Target failures remain failures; no retries.'})
    api = []
    for i in range(10):
        r = json.loads((HERE/f'api_{i:02}_api.json').read_text())
        actual = sha256_file(r['output'])
        if actual != r['reference_sha256']:
            raise AssertionError('API control changed after exit')
        api.append({'name': r['name'], 'status': r['status'], 'counts': r['counts'],
                    'post_exit_sha256': actual, 'peak_rss_kib': r['peak_rss_kib']})
    atomic_json(HERE/'api_summary.json', api)
    table = '\n'.join(f"| {r['group']} | {r['bytes']/2**20:.3f} | {r['failures']}/{r['trials']} | {100*r['failure_fraction']:.0f}% |" for r in rates)
    cases = [r for r in forensic if 'rejected_read_buffer' not in r['file']]
    case_table = '\n'.join(f"| {r['trial']} | {r['bytes']:,} | {r['missing_tail_bytes']:,} | {r['first_divergence_zero_based']:,} (EOF) |" for r in cases)
    report = f'''# Large-write investigation — 8 September 2026
...
'''
    ...
    print('Report written; 95 primary plus 10 API trials; reliability gate remains BLOCKED.')
