...
def schedule(policy,budget):
    ...
    while k<budget:
        ...
        entries+=min(candidates,max(1000,b+100)+size) if policy=='explore_10' else min(candidates,max(1000,b+100))
...
_durable.write_text(ROOT/'phase3b_outputs/estimates.json', json.dumps(result,indent=2)+'\n')
