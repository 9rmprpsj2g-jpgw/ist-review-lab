...
## Known v1 solver defect: sensitivity was unresolved in v1

Provisional pending ledger reconstruction: eight of the 5,862 fits with retained per-fit records reached max_iter=10,000 without converging, all on C15 (R=4,179). The archived run counts total 5,880 fits, but convergence status for 18 fits cannot currently be determined from the retained ledgers: six records are missing from C12/11/auto_tar, five from C12/47/auto_tar, and seven from C16/47/auto_tar. These 18 must not be counted as observed converged fits. Their topics differ from C15, where all eight known failures occurred; this pattern does not establish their convergence or quantify their failure probability. Subsequent reconstruction recovered and verified all 47 records for C12/47 and C16/47, including 12 formerly missing fits, all converged. C12/11 logged a matching 47-record digest inside the reconstruction process but retained only 41 records at post-exit verification. Thus 5,874 fit records are now available across original and verified reconstructed evidence, with eight known failures and six fit statuses still unsupported by retained per-fit records. The reconstruction process reports those six converged, but its conflicting persistence receipts leave the evidence gate failed; the total remains provisional. The original run log contains all eight warnings; earlier phase gates failed to examine them. C15 / seed 11 / fixed_20 fit 209 occurs after 4,161 reviews, upstream of the recorded 90% crossing at 4,283 and committed boundary at 4,301. It therefore contributes to the C15 topic value in auto_tar versus fixed_20, the single uncensored batch_effort_at_90 contrast. This is a known solver defect in one of the five topic values supporting the headline +193.9-document committed-effort difference. **The sensitivity of that contrast to completed optimization is unknown and unresolved as of v1. The affected result must not be described as unaffected.** The historical values below remain visible; they have not been replaced by a diagnostic result.

### Subsequent isolated sensitivity check: fit 209

With the identical training rows/order, temporary negatives, seed, C, loss, dual and tol, C15/11/fixed_20 fit 209 converged after **10,190 iterations** under a diagnostic ceiling of 100,000. Spearman correlation between its converged and original margins over all 18,988 candidates was **0.999999929314762**. The selected 20-document batch was identical in both membership and order. Replaying all 41 remaining fits at the original 10,000 limit produced no further convergence warnings and an exactly identical complete row_order. Effort at 90% remained **4,283** and committed effort remained **4,301**. The isolated change to the reported five-topic/three-seed +193.9-document contrast is **0 documents**. This directly closes the sensitivity concern for completing fit 209 alone. It does not establish that the other seven capped fits are harmless, that prevalence is the cause, or that a particular iteration budget suffices for the census grid. The original eight failures and the historical v1 disclosure remain part of the record.
